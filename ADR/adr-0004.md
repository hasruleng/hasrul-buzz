...
**Option D (skill tree) is kept as an *optional, secondary* component for the recruiter lens** — a
"skill breadth at a glance" panel, *not* the main career layout. It earns its place on the secondary
criterion (good Phase-3 fit → it becomes the game's character-stats panel) and on its actual strength
(showing skill spread), while staying out of the way of readability because it's an opt-in inset, not
...
- **Phase 2 builds A**: a vertical timeline of the stations with click-to-expand project cards — a
  Render-layer change only, feeding off the same `stations.ts` / `projects.ts` / `lenses.ts`
  ([ADR-0001](adr-0001.md), [ADR-0003](adr-0003.md)). Low cost (1–2 days) keeps Phase 2's real focus —
  the shared visual identity ([ADR-0005](adr-0005.md)) — front and centre.
- **D (skill tree) is deferred but not dropped**: built as an *optional* recruiter-lens inset, and it
  becomes the game's character-stats panel in Phase 3. When built, the `skills` field on `LensDef` is a
  flat string array — a real graph would need a normalised skills entity (known limit flagged in
  [ADR-0003](adr-0003.md)). Revisit then, not now.
